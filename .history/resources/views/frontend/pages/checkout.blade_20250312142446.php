...
document.addEventListener("DOMContentLoaded", function() {
    ...
    provinceSelect.addEventListener("change", async function() {
        ...
        try {
            const response = await fetch(`/get-districts?province_code=${provinceCode}`);
            if (!response.ok) throw new Error("Lỗi API quận/huyện");
            const data = await response.json();

            districtSelect.disabled = false;
            districtSelect.innerHTML += data.map(district => 
    ...
    districtSelect.addEventListener("change", async function() {
        ...
        try {
            const response = await fetch(`/get-wards?district_code=${districtCode}`);
            if (!response.ok) throw new Error("Lỗi API xã/phường");
            const data = await response.json();

            wardSelect.disabled = false;
            wardSelect.innerHTML += data.map(ward => 
