        ...
        < script >
            $(document).ready(function() {
                $('#shippingSelect').on('change', function() {
                    // Lấy giá trị của lựa chọn hiện tại
                    const selectedOption = $(this).find(':selected');
                    const type = selectedOption.data('type'); // Lấy phương thức giao hàng

                    // Cập nhật giá trị của trường địa chỉ 1 và ngăn không cho xóa hoặc thay đổi phần đã nhập
                    const address1Value = `${type}`;
                    const currentValue = $('#address1').val();

                    // Nếu input đã có giá trị, chỉ thêm phần mới vào phía trước
                    if (currentValue === "") {
                        $('#address1').val(address1Value);
                    } else if (!currentValue.startsWith(address1Value)) {
                        $('#address1').val(address1Value + currentValue);
                    }
                });

                // Ngăn không cho xóa hoặc sửa trực tiếp trong input (chỉ có thể thay đổi ở phía trước)
                $('#address1').on('keydown', function(e) {
                    const currentValue = $(this).val();

                    // Không cho phép xóa (dấu backspace và delete)
