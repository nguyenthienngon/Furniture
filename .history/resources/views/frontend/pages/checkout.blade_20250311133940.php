                                ...
                                <script>
                                    ...
                                    function loadProvinces() {
                                        ...
                                        if (locations.length === 0) {
                                            console.error('Không có dữ liệu tỉnh nào được tải.');
                                            alert('Không có dữ liệu tỉnh nào. Vui lòng kiểm tra file address.json.');
                                            return;
                                        }
                                        ...
                                        console.log('Đã tải xong danh sách tỉnh:', locations.length, 'tỉnh');
                                    }
                                    ...
                                    window.onload = function() {
                                        console.log('Bắt đầu tải file address.json từ public/js/address.json...');
                                        fetch('public/js/address.json')
                                            .then(response => {
                                                console.log('Phản hồi từ server:', response.status, response.statusText);
                                                if (!response.ok) {
                                                    throw new Error(`Không thể tải file JSON: ${response.status} - ${response.statusText}`);
                                                }
                                                return response.json();
                                            })
                                            .then(data => {
                                                locations = data;
                                                console.log('Dữ liệu JSON đã tải:', locations);
                                                loadProvinces();
                                            })
                                            .catch(error => {
                                                console.error('Lỗi khi tải file:', error);
                                                alert('Không thể tải dữ liệu địa chỉ. Kiểm tra console để biết chi tiết.');
                                            });
                                    };
                                </script>
