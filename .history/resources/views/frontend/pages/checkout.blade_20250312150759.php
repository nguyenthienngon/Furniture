...
<select id="district" name="district" onchange="handleDistrictChange(this)" disabled>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    window.handleProvinceChange = function (selectElement) {
        ...
        // Xóa danh sách cũ và disable dropdown
        ...
        defaultDistrictOption.textContent = "-- Chọn huyện --";
        districtSelect.appendChild(defaultDistrictOption);

        let defaultWardOption = document.createElement('option');
        defaultWardOption.value = "";
        defaultWardOption.textContent = "-- Chọn xã --";
        wardSelect.appendChild(defaultWardOption);

        if (!provinceCode) return;
        console.log("Filtered Districts:", filteredDistricts);
console.log("districtSelect.disabled trước khi mở:", districtSelect.disabled);

        // Kiểm tra danh sách huyện có dữ liệu không
        ...
        if (filteredDistricts.length > 0) {
            ...
            console.warn("Không tìm thấy huyện nào cho tỉnh này!");
        }
    };
});
</script>
