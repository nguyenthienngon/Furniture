...
@push('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            fetch('http://127.0.0.1:8001/js/address.json')
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        ...
                        console.error("Dữ liệu JSON rỗng!");
                    }
                })
            ...
            function loadProvinces(data) {
                ...
                provinceSelect.addEventListener("change", function() {
                    ...
                    console.log("Mã tỉnh đã chọn:", provinceCode);

                    let selectedProvince = data.find(p => p.Code === provinceCode);
                    if (selectedProvince) {
                        loadDistricts(selectedProvince.District);
                    } else {
                        console.warn("Không tìm thấy tỉnh phù hợp!");
                        loadDistricts([]);
                    }
                });
            }
            ...
            function loadDistricts(districts) {
                ...
                if (!districts || districts.length === 0) {
                    console.warn("Danh sách quận/huyện rỗng!");
                    return;
                }
                ...
                districtSelect.addEventListener("change", function() {
                    ...
                    console.log("Mã quận đã chọn:", districtCode);

                    let selectedDistrict = districts.find(d => d.Code === districtCode);
                    if (selectedDistrict) {
                        loadWards(selectedDistrict.Ward);
                    } else {
                        console.warn("Không tìm thấy quận phù hợp!");
                        loadWards([]);
                    }
                });
            }
            ...
            function loadWards(wards) {
                ...
                if (!wards || wards.length === 0) {
                    console.warn("Danh sách phường/xã rỗng!");
                    return;
                }
