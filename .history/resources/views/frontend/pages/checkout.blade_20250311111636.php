...
@section('main-content')
    ...
    <script>
        console.log("Dữ liệu allProvinces:", allProvinces);

        let allProvinces = @json($provinces); // Chuyển dữ liệu từ PHP sang JavaScript

        $("#province").change(function() {
            let provinceCode = $(this).val().toString(); // Lấy mã tỉnh được chọn (chuỗi)
            console.log("Mã tỉnh được chọn:", provinceCode); // In mã tỉnh ra console

            let selectedProvince = allProvinces.find(p => p.Code.toString() === provinceCode);

            console.log("Tỉnh được chọn: ", selectedProvince ? selectedProvince.FullName : "Không tìm thấy tỉnh");

            // Tìm tất cả huyện có ProvinceCode trùng với mã tỉnh đã chọn
            let matchingDistricts = selectedProvince?.District || [];
            console.log("Danh sách huyện có ProvinceCode =", provinceCode, ":", matchingDistricts);
            ...
            if (matchingDistricts.length > 0) {
                ...
                console.log("Không tìm thấy huyện nào cho tỉnh này.");
            }
        });
    </script>
