                        ...
                        <div class="checkout-form">
                            ...
                            <script>
                                $(document).ready(function() {
                                    let allProvinces = @json($provinces);
                                    ...
                                    $(document).on("change", "#province", function() {
                                        ...
                                        // Xóa danh sách cũ và thêm option mặc định
                                        districtSelect.empty().append('<option value="">Chọn huyện</option>');
                                        wardSelect.empty().append('<option value="">Chọn xã</option>');
                                        console.log("Mã HTML của danh sách huyện:", districtSelect.html());

                                        if (!selectedProvince) return;

                                        let districts = selectedProvince["Districts"] || [];

                                        // Hiển thị danh sách huyện trên trang
                                        let districtOptions = districts.map(district =>
                                            `<option value="${district.Code}">${district.FullName}</option>`
                                        ).join('');
                                        districtSelect.append(districtOptions);

                                        // Nếu có huyện đầu tiên, tự động load danh sách xã của huyện đó
                                        if (districts.length > 0) {
                                            let firstDistrict = districts[0];
                                            districtSelect.val(firstDistrict.Code).trigger("change");
                                        }
                                    });

                                    $(document).on("change", "#district", function() {
                                        let districtCode = $(this).val();
                                        let provinceCode = $("#province").val().trim();

                                        let selectedProvince = allProvinces[provinceCode] || null;
                                        if (!selectedProvince) return;

                                        let districts = selectedProvince["Districts"] || [];
                                        let selectedDistrict = districts.find(d => d.Code === districtCode) || null;
                                        if (!selectedDistrict) return;

                                        let wards = selectedDistrict["Ward"] || [];

                                        // Hiển thị danh sách xã trên trang
                                        let wardOptions = wards.map(ward =>
                                            `<option value="${ward.Code}">${ward.FullName}</option>`
                                        ).join('');
                                        $("#ward").empty().append('<option value="">Chọn xã</option>' + wardOptions);
                                    });
                                });
                            </script>
