...
<select id="province" name="province">
    ...
    @foreach($addressData as $province)
        <option value="{{ $province['Code'] }}" data-province-code="{{ $province['Code'] }}">{{ $province['FullName'] }}</option>
    @endforeach
</select>
        ...
        @if(isset($province['District']) && is_array($province['District']))
            @foreach($province['District'] as $district)
                ...
                @else
                    ...
                    <option value="" style="display: none;" disabled>Huyện không hợp lệ</option>
                @endif
            @endforeach
        @endif
    @endforeach
                ...
                @if(isset($district['Ward']) && is_array($district['Ward']) && isset($district['DistrictCode']))
                    @foreach($district['Ward'] as $ward)
                        ...
                        @else
                            ...
                            <option value="" style="display: none;" disabled>Xã không hợp lệ</option>
                        @endif
                    @endforeach
                @endif
            @endforeach
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    provinceSelect.addEventListener('change', function () {
        ...
        if (provinceCode) {
            // Lấy tất cả các option huyện
            const districtOptions = Array.from(districtSelect.querySelectorAll('option[data-province-code]'));
            
            // Lọc và hiển thị các huyện thuộc tỉnh đã chọn
            districtOptions.forEach(option => {
                if (option.getAttribute('data-province-code') === provinceCode) {
    ...
    districtSelect.addEventListener('change', function () {
        ...
        if (districtCode) {
            // Lấy tất cả các option xã
            const wardOptions = Array.from(wardSelect.querySelectorAll('option[data-district-code]'));

            // Lọc và hiển thị các xã thuộc huyện đã chọn
            wardOptions.forEach(option => {
                if (option.getAttribute('data-district-code') === districtCode) {
                    const newOption = document.createElement('option');
                    newOption.value = option.value;
                    newOption.textContent = option.textContent;
