...
<script>
    ...
    // Hiển thị alert khi dữ liệu provinces được load
    alert("Dữ liệu provinces đã tải: " + JSON.stringify(provinces, null, 2));
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        provinces.forEach(province => {
            provinceSelect.innerHTML += `<option value="${province.code}">${province.full_name}</option>`;
        });

        // Khi chọn tỉnh/thành phố
        provinceSelect.addEventListener("change", function() {
            const provinceCode = this.value.trim();
            console.log("Selected Province Code:", provinceCode);

            // Reset danh sách quận/huyện và xã/phường
            ...
            // Lọc danh sách quận/huyện từ dữ liệu đã có
        ...
        districtSelect.addEventListener("change", function() {
            const districtCode = this.value.trim();
            ...
            // Lọc danh sách xã/phường từ dữ liệu đã có
