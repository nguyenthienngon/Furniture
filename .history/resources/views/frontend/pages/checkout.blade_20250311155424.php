                            ...
                            <div class="row">
                                ...
                                <select id="province" name="province">
                                    ...
                                    @foreach ($addressData as $province)
                                        <option value="{{ $province['Code'] }}"
                                            data-province-code="{{ $province['Code'] }}">{{ $province['FullName'] }}
                                        </option>
                                    @endforeach
                                </select>
                                    ...
                                    @foreach ($addressData as $province)
                                        @if (isset($province['District']) && is_array($province['District']))
                                            @foreach ($province['District'] as $district)
                                                @if (isset($district['DistrictCode']) && isset($district['FullName']))
                                                ...
                                                @else
                                                    <!-- Bỏ qua hoặc hiển thị mặc định nếu không có DistrictCode -->
                                                    <option value="" style="display: none;" disabled>Huyện không hợp
                                                        lệ</option>
                                                @endif
                                            @endforeach
                                        @endif
                                ...
                                <select id="ward" name="ward" disabled>
                                    ...
                                    @foreach ($addressData as $province)
                                        @if (isset($province['District']) && is_array($province['District']))
                                            @foreach ($province['District'] as $district)
                                                @if (isset($district['Ward']) && is_array($district['Ward']) && isset($district['DistrictCode']))
                                                    @foreach ($district['Ward'] as $ward)
                                                        @if (isset($ward['WardCode']) && isset($ward['FullName']))
                                                        ...
                                                        @else
                                                            <!-- Bỏ qua hoặc hiển thị mặc định nếu không có WardCode -->
                                                            <option value="" style="display: none;" disabled>Xã không
                                                                hợp lệ</option>
                                                        @endif
                                                    @endforeach
                                                @endif
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            // Lắng nghe sự kiện thay đổi của dropdown "province"
            $('#province').on('change', function() {
                ...
                districtSelect.html('<option value="">-- Chọn huyện/quận --</option>').prop('disabled',
                    true);
                wardSelect.html('<option value="">-- Chọn xã/phường --</option>').prop('disabled', true);

                if (provinceCode) {
                    // Lọc và hiển thị các huyện thuộc tỉnh đã chọn
                    $('#district option[data-province-code]').each(function() {
                        if ($(this).data('province-code') == provinceCode) {
                            $(this).show(); // Hiển thị option phù hợp
                        } else {
                            $(this).hide(); // Ẩn các option không liên quan
                        }
                    });

                    // Kích hoạt dropdown huyện nếu có dữ liệu
                    if ($('#district option[data-province-code="' + provinceCode + '"]').length > 0) {
                        districtSelect.prop('disabled', false);
                    }
                }
            });

            // Lắng nghe sự kiện thay đổi của dropdown "district"
            $('#district').on('change', function() {
                ...
                if (districtCode) {
                    ...
                    $('#ward option[data-district-code]').each(function() {
                    ...
                    if ($('#ward option[data-district-code="' + districtCode + '"]').length > 0) {
