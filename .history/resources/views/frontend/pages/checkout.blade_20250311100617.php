...
@section('main-content')
    ...
    <script>
        var locations = @json($data['provinces']);

        $(document).ready(function() {
            console.log("Dữ liệu tỉnh thành:", locations);
            ...
            $('#province').change(function() {
                ...
                if (provinceId) {
                    var province = locations.find(p => p.Code == provinceId);
                    if (!province) {
                        console.error("Không tìm thấy tỉnh với ID:", provinceId);
                        return;
                    }

                    console.log("Tỉnh được chọn:", province.FullName);
                    console.log("Danh sách quận/huyện:", province.Districts);
            ...
            $('#district').change(function() {
                ...
                if (districtId) {
                    ...
                    var district = province.Districts.find(d => d.Code == districtId);
                    if (!district) {
                        console.error("Không tìm thấy huyện với ID:", districtId);
                        return;
                    }

                    console.log("Huyện được chọn:", district.FullName);
                    console.log("Danh sách xã/phường:", district.Wards);

                    $.each(district.Wards, function(index, ward) {
                        $('#ward').append('<option value="' + ward.Code + '">' + ward.FullName +
                            '</option>');
                    });
