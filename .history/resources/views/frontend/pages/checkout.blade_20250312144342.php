...
<section class="shop checkout section">
  ...
  <select id="province" name="province" onchange="handleProvinceChange(this)">
...
<select id="district" name="district" disabled onchange="handleDistrictChange(this)">
...
@endpush
<pre>{{ print_r($districts, true) }}</pre>
...
document.addEventListener("DOMContentLoaded", function() {
   ...
   window.handleProvinceChange = function(selectElement) {
    ...
    // Lọc danh sách huyện theo tỉnh (đảm bảo so sánh kiểu string)
    const filteredDistricts = districts.filter(d => String(d.province_code) === String(provinceCode));
    
    console.log("Filtered Districts:", filteredDistricts); // Kiểm tra kết quả
    ...
    // Khi chọn huyện/quận
    window.handleDistrictChange = function(selectElement) {
        const districtCode = selectElement.value;
        console.log("Selected District:", districtCode);

        // Reset danh sách xã/phường
        wardSelect.innerHTML = '<option value="">-- Chọn xã/phường --</option>';
        wardSelect.disabled = true;

        if (!districtCode) return;

        // Lọc danh sách xã theo huyện
