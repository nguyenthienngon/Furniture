...
class CartController extends Controller
{
    ...
    public function checkout()
    {
        ...
        if (!$data) {
            abort(500, "Lỗi đọc dữ liệu địa chỉ");
        }

        // Chuẩn hóa dữ liệu: Chuyển tất cả mã tỉnh về dạng chuỗi
        $provinces = [];
        foreach ($data as $province) {
            $provinceCode = (string) $province['Code']; // Ép kiểu về string
            $provinces[$provinceCode] = [
                'FullName' => $province['FullName'],
                'Districts' => isset($province['District']) ? $province['District'] : [] // Giữ nguyên cấu trúc District
            ];
        }
        ...
        return view('frontend.pages.checkout', compact('totalAmount', 'discount', 'grandTotal', 'provinces'));
    }
}
