...
class CartController extends Controller
{
    ...
    {
        $jsonPath = public_path('js/address.json'); // Đọc JSON từ thư mục public/js
        $addressData = json_decode(file_get_contents($jsonPath), true);

        // Kiểm tra dữ liệu
        if (!file_exists($jsonPath)) {
            Log::error("Không tìm thấy file JSON tại: " . $jsonPath);
            $addressData = [];
        } elseif (empty($addressData)) {
            Log::error("Dữ liệu JSON trống hoặc không hợp lệ.");
        }
        ...
        return view('frontend.pages.checkout', compact('totalAmount', 'discount', 'grandTotal', 'addressData'));
    }
}
