...
class RevenueReportExport implements FromView
{
    ...
    {
        $orderQuery = Order::where('orders.status', 'delivered'); // Chỉ lấy đơn hàng đã giao
        if ($this->from_date && $this->to_date) {
            $orderQuery->whereBetween('orders.created_at', [$this->from_date, $this->to_date]);
        }

        // Lấy danh sách các ngày trong khoảng thời gian
        $start = \Carbon\Carbon::parse($this->from_date);
        $end = \Carbon\Carbon::parse($this->to_date);
        $allDates = [];
        while ($start->lte($end)) {
        ...
        $categoryRevenue = Order::join('order_details', 'orders.id', '=', 'order_details.order_id')
            ...
            ->where('orders.status', 'delivered') // Lọc đơn hàng đã giao
        ...
        $topSellingProducts = Order::join('order_details', 'orders.id', '=', 'order_details.order_id')
            ...
            ->limit(5);
        $orderStatusReport = Order::selectRaw('status, COUNT(*) as count, SUM(total_amount) as revenue')
            ->whereIn('status', ['delivered', 'new', 'cancel', 'process'])
            ->groupBy('status')
            ->get()
            ->keyBy('status');
        ...
        if ($this->from_date && $this->to_date) {
            ...
            $orderStatusReport->whereBetween('orders.created_at', [$this->from_date, $this->to_date]);
        }
